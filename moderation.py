...
class Moderation(commands.Cog):
    ...
    async def notes(self, ctx: commands.Context, member: discord.Member):
        db = get_db()
        cursor = db.notes.find({"guild_id": ctx.guild.id, "user_id": member.id})
        notas = await cursor.to_list(length=50)
        if not notas:
            await ctx.send(f"{member.mention} no tiene notas guardadas.")
            return
        embed = discord.Embed(title=f"{EMOJI_NOTE} Notas de {member}", color=0x2F3136)
        for n in notas:
            embed.add_field(
                name=f"ID: {n['_id']}",
                value=f"{n['note']}\nModerador: <@{n['moderator_id']}>",
                inline=False,
            )
        await ctx.send(embed=embed)
...
async def setup(bot: commands.Bot):
    await bot.add_cog(Moderation(bot))import discord
import datetime
from discord.ext import commands
from database import get_db
